...
root=Tk()
root.title('Heart Health Monitoring System')
...
def analysis():
    ...
    if E > 200:
        messagebox.showinfo('High Cholesterol', 'Your cholesterol level is high. Consider exercising or eating healthier to lower your cholesterol.')
    
    if D < 120:
        messagebox.showinfo('Resting Blood Pressure', 'Your resting blood pressure is normal.')
    elif 120 <= D <= 139:
        messagebox.showinfo('Resting Blood Pressure', 'Your resting blood pressure indicates prehypertension.')
    elif 140 <= D <= 159:
        messagebox.showinfo('Resting Blood Pressure', 'Your resting blood pressure indicates high blood pressure (hypertension).')
    elif D >= 160:
        if D < 180:
            messagebox.showinfo('Resting Blood Pressure', 'Your resting blood pressure indicates hypertension stage 2.')
        else:
            messagebox.showinfo('Resting Blood Pressure', 'Your resting blood pressure indicates hypertensive crisis. Emergency care is needed.')
        
        
    if thalach.get() > 180:
        messagebox.showinfo('High Maximum Heart Rate', 'Your maximum heart rate achieved is high. Please consult with a healthcare professional.')

    if cp_combobox.get() == '3 = asymptomatic':
        messagebox.showinfo('Asymptomatic Chest Pain', 'Asymptomatic chest pain may indicate a risk of heart disease. Please consult with a healthcare professional.')

    if ca_combobox.get() == '4':
        messagebox.showinfo('High Number of Major Vessels', 'A high number of major vessels may indicate severe heart disease. Please consult with a healthcare professional.')

    # Additional checks and warnings
    if trestbps.get() > 140:
        messagebox.showinfo('High Resting Blood Pressure', 'Your resting blood pressure is high. Please monitor it regularly and consult with a healthcare professional.')

    if oldpeak.get() > 2.0:
        messagebox.showinfo('Significant ST Depression', 'A significant ST depression induced by exercise relative to rest may indicate heart disease. Please consult with a healthcare professional.')

    if thal_combobox.get() == '2':
        messagebox.showinfo('Reversible Defect', 'A reversible defect detected in thalassemia test may indicate heart disease. Please consult with a healthcare professional.')    
    ...
    ####First graph
    f = Figure(figsize=(5,5),dpi=100)  #5x5inches 
    a = f.add_subplot(111)      #1x1 grid
    a.plot(["Sex","fbs","exang"],[B,F,I])
    canvas = FigureCanvasTkAgg(f)       #display matplotfigure
    canvas.get_tk_widget().pack(side=tk.BOTTOM,fill=tk.BOTH,expand=True)     #packs the Tkinter canvas widget into the Tkinter GUI window
    ...
    if(prediction[0]==0):
        ...
        print('The person is at greater risk of having Heart disease')
        report.config(text=f"Report:{0}",fg="#ed1c24")
        report1.config(text=f"{name},you are at greater risk of having a heart disease")
...
def Save():
    ...
    Save_Data_MySql(B2,C2,int(D2),int(E2),int(F2),int(G2),int(H2),int(I2),int(J2),int(K2),int(L2),int(M2),int(N2),int(O2),int(P2),int(Q2),str(R2))
...
#icon 1 heart icon on left of title
...
def selection4():
    ...
    if input == '0 = typical angina' or input == '1 = atypical angina' or input == '2 = non-anginal pain' or input == '3 = asymptomatic':
        return input.split(' = ')[0]  # Return the numerical value before the '=' sign
    else:
        messagebox.showerror('missing', 'Please select cp!!')
...
Button(root,image=save_button,bd=0,bg=background,cursor='hand2',command=Save).place(x=470,y=300)
